...
function getPolynom(...args) {
  ...
  return (x) => args.reduce(
    (result, value, index) => (result || 0) + value * x ** (args.length - index - 1),
    null,
  );
}
...
function logger(func, logFunc) {
  return (...args) => {
    const funcCallDescription = `${func.name}(${JSON.stringify(args).slice(1, -1)})`;
    // log before
    logFunc(`${funcCallDescription} starts`);

    // call original function
    const result = func(...args);

    // log after
    logFunc(`${funcCallDescription} ends`);
...
function partialUsingArguments(fn, ...args) {
  // return (...remainingArgs) => fn(...args.concat(remainingArgs));

  const innerFunc = (...remainingArgs) => fn(...args.concat(remainingArgs));
